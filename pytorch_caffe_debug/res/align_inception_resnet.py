...
class Inception_Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv2_2 = nn.Conv2d(int(planes * 0.5), int(planes * 0.5), kernel_size=3, stride=stride, padding=1,
                                 bias=False)
...
def align_inception_resnet101():
    ...
    """
    model = Align_Inception_ResNet([3, 4, 23, 3])
    # _model = torch.load('/home/yanglu/merge_bn_scale/jjn/pytorch2caffe-resx/irb/align_inception_resnet101-0709.caffemodel')
    # _model = _model['state_dict']
    # model.load_state_dict(_model)
    pretrained_model = {}
    model_path = '/home/yanglu/merge_bn_scale/jjn/pytorch2caffe-resx/irb/model_best.pth.tar'
    _model = torch.load(model_path)
    # print(_model['state_dict']['module.conv1.weight'])

    for key in _model['state_dict'].keys():
        pretrained_model[key.replace('module.', '')] = _model['state_dict'][key]
